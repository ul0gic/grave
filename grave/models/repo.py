...
(`api._normalize_item`) and consumed by persistence and display. Lives in
its own module so both the API layer and downstream consumers can import it
without forming an import cycle (presets imports api, which would otherwise
have to host the type).
"""
...
class RepoItem(TypedDict):
    ...
    Every key is always present: `api._normalize_item` builds the dict as a
